    ...
    public class Canvas
    {
        ...
        public void Render(Scene scene) // This function creates the visual element of the figure generated from the points added
        {
            ...
            for (int f = 0; f < scene.Figures.Count; f++)
            {
                ...
                {
                    g.FillPolygon(Brushes.DarkGreen, scene.Figures[f].Pts.ToArray());
                    g.DrawPolygon(Pens.OrangeRed, scene.Figures[f].Pts.ToArray());
                    ...
                    g.FillEllipse(Brushes.OrangeRed, scene.Figures[f].Centroid.X - 3, scene.Figures[f].Centroid.Y - 3, 6, 6);//*/
                }
            }
            pct.Invalidate();
        }
